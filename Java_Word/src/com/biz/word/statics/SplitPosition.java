 ...
 * static 키워드가 부착이 되면
 * 이 클래스를 static믈래스라고 한다.
 * 
 * static 클래스는 초기화 과정을 수행할 필요 없이 사용할수 있다.
 * 
 * SplitPosition sPos = new SplitPosition(); 와 같은 코드를 사용할 필요가 없다.
 * 
 * 
 * static 클래스는 프로젝트가 run(시작)되는 순간
 * 자동으로 객체를 한번 생성하여 method 기억영역에 보관해 둔다.
...
public class SplitPosition {
	...
	// 만약 WORD_ENG 변수를 어딘가에서 0이아닌 다른 값으로 변경을 해 버리면 
	// WORD_ENG변수를 진짜 사용해야 할 곳에서 문제가 발생할 수 잇따.
