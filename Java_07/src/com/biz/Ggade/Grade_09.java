...
public class Grade_09 {
	...
	public static void main(String[] args) {
		...
		for (int i = 1; i <= 2; i++) {
			/*
			 * for() 가 반복되면서 사용되는 변수 i 를 다른 변수에 부착하여 아래와 같이 사용하면 num1 = 90; 이라는 코드로 바뀌어 사용할
			 * 수 있을 것 같은데 문법상 이 코드는 num i 라는 변수를 찾는 코드로 변환되어 syntax 오류를 발생한다. 이러한 코드는 사용이
			 * 불가능하다.
			 */
			// numi = 90;
		}
		...
		//배열 100개에 임의 점수 51 ~ 100 까지 채워 넣기
		for (int i = 0; i < 100; i++) {
		...
		for (int i = 0; i < 100; i++) {
			System.out.println(i + "번째점수" + intKorScore[i]);
