...
public class NewClass_01 {
	 ...
	 * 자바에서 클래스를 만들게 되면 모든 클래스에는 자신의 이름과 같은 method 가
	 * 자동으로 생성된다.
	 * 객체를 초기화할때 이 생성자를 호출하여 초기화를 수행하는 것이다.
	 * 
	 * 생성자를 임으로 작성하고자 할 때에는
	 * 	접근제한자를 public으로 설정하고
	 * 	return type(void등등)이 없다.
	 *	 클래스의 이름과 대소문자가 일치하도록 한다
	 * 
	 * 생성자를 임의로 (재)작성하는 이유
